  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface[]>(
    ...
    () =>
      getBusinesses({
        relations: [
          'user',
          'financial_detail.count',
          'financial_status.count',
          'loan_application.count',
          'loan_detail.count',
        ],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('financial_status', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.financial_status}</Td>
                    )}
                    {hasAccess('loan_application', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.loan_application}</Td>
                    )}
                    {hasAccess('loan_detail', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.loan_detail}</Td>
                    )}
                    <Td>
